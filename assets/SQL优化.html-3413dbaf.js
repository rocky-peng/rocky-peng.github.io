...
SWAPS：显示交换次数相关开销
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li>`,4),_=s(`<h4 id="注意" tabindex="-1"><a class="header-anchor" href="#注意" aria-hidden="true">#</a> 注意</h4><p>如果在其生命周期阶段出现如下的情况的就要重视了：</p><ol><li><p>converting HEAP to MyISAM 表示正在将基于内存的临时表转储到基于磁盘的临时表上</p><p>MySQL会使用Memory存储引擎的表来作为临时表来存放查询产生的中间结果，如果中间结果集大于了memory存储引擎表的容量（tmp_table_size参数控制），或者中间结果包括了memory存储引擎表不支持的数据类型，mysql又将此中间结果转换为MyISAM存储引擎表而存放在磁盘上。而MyISAM表是不缓存数据文件的，因此产生这样的临时表对于查询的性能会有所降低，所以在查询时尽量避免产生临时表。</p></li><li><p>Creating tmp table 创建了临时表，先拷贝到临时表，用完后再删除</p></li><li><p>Copying to tmp table on disk 把内存中的临时表复制到磁盘中，这个很耗性能</p></li><li><p>Locked 这个就是指在等待锁的意思</p></li></ol><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>查看全局创建的内存临时表数量
SHOW GLOBAL STATUS LIKE &#39;Created_tmp_tables&#39;;

查看全局创建的磁盘临时表数量
SHOW GLOBAL STATUS LIKE &#39;Created_tmp_disk_tables&#39;;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="sql使用技巧" tabindex="-1"><a class="header-anchor" href="#sql使用技巧" aria-hidden="true">#</a> SQL使用技巧</h2><ol><li>是否有索引</li><li>覆盖索引 <ul><li>可以避免回表操作</li></ul></li><li>避免隐式的类型转换</li><li>连表的两个字段必须保持相同的数据类型和字符集 <ul><li>有可能不能使用索引，造成全表扫描</li></ul></li><li>not exist与not in(优先选择not exist) <ul><li>not in使用不当还有可能返回错误结果。 如果not in子查询包含了null，将返回空数据。 如果not in子查询返回空数据，将匹配所有行数据</li><li>not in (1,null) : 返回空数据</li><li>not in (select 1 where 1=0) : 返回所有数据</li></ul></li><li>索引字段进行表达式或函数计算 <ul><li>会造成全表扫描</li></ul></li><li>不满足左匹配原则，比如模糊查询 <ul><li>会造成全表扫描</li></ul></li><li>!=（不等于） <ul><li>会造成全表扫描</li></ul></li><li>索引字段尽量建立在区分度高的列上 <ul><li>比如在is_del这样的字段上加索引意义就不是很大</li></ul></li><li>长字段单独一张表</li></ol><ul><li>从b+数的结构上解释，一页的空间是16kb，存放的数据占用空间越小那么一页存放的数据越多，整棵树就更加矮胖，IO次数就越少，效率越高</li></ul>`,7),E={start:"11"},L={href:"https://mp.weixin.qq.com/s/KpTG1WUlcP6cyk8Y9SMcyQ",target:"_blank",rel:"noopener noreferrer"},y=s("<li>小表驱动大表</li><li>时间排序转换为id排序</li><li>分页技巧（断点分页）</li><li>表设计时，满足要求的情况下尽量选择占用空间小的数据类型</li><li>合理适用子查询，比如in与exist，有时比连表效率更高</li><li>sql语句尽量简单（这个我的使用习惯），一些操作可以让应用层完成</li><li>聚合操作避免使用ICP（也就是聚合操作的extra列不要出现using index condition）</li><li>or</li><li>null</li><li>避免使用not in,而用not exists 替代</li>",10),I=e("h2",{id:"icp-index-condition-pushdown",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#icp-index-condition-pushdown","aria-hidden":"true"},"#"),i(" ICP(Index Condition PushDown)")],-1),w={href:"https://www.jb51.net/article/222444.htm",target:"_blank",rel:"noopener noreferrer"},R=e("h2",{id:"mysql执行计划",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#mysql执行计划","aria-hidden":"true"},"#"),i(" MySQL执行计划")],-1),N={href:"https://blog.csdn.net/horses/article/details/106905110",target:"_blank",rel:"noopener noreferrer"},A=s('<h2 id="小表驱动大表" tabindex="-1"><a class="header-anchor" href="#小表驱动大表" aria-hidden="true">#</a> 小表驱动大表</h2><h3 id="何为小表驱动大表" tabindex="-1"><a class="header-anchor" href="#何为小表驱动大表" aria-hidden="true">#</a> 何为小表驱动大表</h3><p>用小的数据集驱动大的数据集</p><h3 id="如何区分驱动表与被驱动表" tabindex="-1"><a class="header-anchor" href="#如何区分驱动表与被驱动表" aria-hidden="true">#</a> 如何区分驱动表与被驱动表</h3><p>很简单：explain 出来的结果中，第一行显示的表即为驱动表</p><h3 id="join的时候谁是驱动表" tabindex="-1"><a class="header-anchor" href="#join的时候谁是驱动表" aria-hidden="true">#</a> join的时候谁是驱动表</h3><ol><li>A left join B，则左表是驱动表，即A是驱动表</li><li>A right join B，则右表是驱动表，即B是驱动表</li><li>A inner join B，MySQL会选择比较小的表为驱动表</li></ol><h3 id="exsit与in" tabindex="-1"><a class="header-anchor" href="#exsit与in" aria-hidden="true">#</a> Exsit与In</h3><p>现在有两张表：</p><p>dept表：部门表，大概20条记录</p><p>emp表：员工表，大概1W条记录</p><p>需求：找出部门名称包含&quot;技术&quot;两个字的所有员工信息</p>',12),x={href:"http://B.name",target:"_blank",rel:"noopener noreferrer"},q={href:"http://B.name",target:"_blank",rel:"noopener noreferrer"},T=e("p",null,"先说结论：",-1),S=e("ol",null,[e("li",null,"当A表的数据集大于B表时，IN比exists更优"),e("li",null,"当A表的数据集小于B表时，exists比IN更优")],-1),U=e("p",null,"根据上面的结论，可知方案一比方案二时间更短，不信可自行实验",-1),M={href:"https://mp.weixin.qq.com/s/KpTG1WUlcP6cyk8Y9SMcyQ",target:"_blank",rel:"noopener noreferrer"},k={href:"https://blog.csdn.net/qq_38011415/article/details/103304189",target:"_blank",rel:"noopener noreferrer"},B=e("h2",{id:"哪些情况会出现全表扫描",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#哪些情况会出现全表扫描","aria-hidden":"true"},"#"),i(" 哪些情况会出现全表扫描")],-1),D=e("li",null,"数据量太小了，走索引可能成本还要高一些",-1),P=e("li",null,"筛选列没有索引",-1),O=e("li",null,"筛选列有索引，但是匹配的数据量接近于整张表数据",-1),C=e("li",null,"筛选列有索引，但是使用方式不对，比如对列进行计算、不满足最左匹配原则",-1),j={href:"https://blog.csdn.net/horses/article/details/107243447",target:"_blank",rel:"noopener noreferrer"},Y=e("h3",{id:"in-和-exist语句",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#in-和-exist语句","aria-hidden":"true"},"#"),i(" IN 和 EXIST语句")],-1),X={href:"https://mp.weixin.qq.com/s/KpTG1WUlcP6cyk8Y9SMcyQ",target:"_blank",rel:"noopener noreferrer"},F=s(`<h2 id="use-index-与-force-index的区别" tabindex="-1"><a class="header-anchor" href="#use-index-与-force-index的区别" aria-hidden="true">#</a> use index 与 force index的区别？</h2><p>use index是告诉优化器可以使用这个索引，但最终优化器可以不用这个索引。</p><p>force index是强制让优化器使用这个索引</p><h2 id="show-index-from-库名-表名" tabindex="-1"><a class="header-anchor" href="#show-index-from-库名-表名" aria-hidden="true">#</a> show index from 库名.表名</h2><p>这条命令输出了一个关键值：cardinality，表示该索引中唯一值的数目。这个值和表总行数应该尽量接近1，否则可以考虑是否可以删除这个索引。但这个值不是实时更新的，只是一个大概值。如果要手动更新这个值，可以使用 analyze table table_name来完成。</p><p>mysql是通过采样的方法来进行统计这个值的</p><p>优化器会根据这个值来判断是否使用这个索引，但这个值可能有时候不准确，因此可能影响sql执行性能，所以可以考虑在系统非高峰时间对核心表执行analyze table操作。</p><h2 id="实际例子" tabindex="-1"><a class="header-anchor" href="#实际例子" aria-hidden="true">#</a> 实际例子</h2><ul><li>MySQL的大部分索引（PRIMARY KEY,UNIQUE,INDEX）都采用的是B-Tree数据结构</li><li>对于一些特定的数据类型，会采用R-Tree数据结构</li><li>内存表（MEMORY table）支持 哈希表 数据结构</li><li>组合索引，复合索引，联合索引，是一个东西 <ul><li>对多个列建立一个索引，如下的组合索引。</li><li>要根据业务场景来决定列的顺序。</li><li>是否能用到联合索引，要看where条件是否符合 <em><strong>最左前缀</strong></em> 原则。</li><li>组合索引的第一个字段必须出现在查询组句中，这个索引才会被用到。（和上一条一个意思）</li></ul><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>create table test(
    a int,
    b int,
    c int,
    KEY a(a,b,c)
...
dubbo-admin  	2.5.3
es				1.6.0-2.1.0
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><br>`,19),H=e("p",null,[e("br"),e("br"),e("br"),e("br"),e("br"),e("br")],-1),W=e("hr",null,null,-1),Q=e("hr",null,null,-1),V=e("ul",null,[e("li",null,[e("strong",null,"随机毒鸡汤"),i("：世界那么大，我的父母应该去看看。 "),e("br"),e("br"),e("img",{src:"https://tuapi.eees.cc/api.php?category=biying&type=302&uuid=db4f759b-d271-4064-8ed4-409c1268c47f",alt:"",loading:"lazy"})])],-1);function G(K,J){const n=v("ExternalLinkIcon");return a(),r("div",null,[t,e("p",null,[i("扩展阅读： "),e("a",u,[i("https://blog.csdn.net/horses/article/details/106905110"),l(n)])]),m,e("p",null,[i("使用performance_schema数据库的具体步骤参考："),e("a",b,[i("https://dev.mysql.com/doc/refman/5.7/en/performance-schema-query-profiling.html"),l(n)])]),o,e("ol",null,[e("li",null,[p,g,e("p",null,[i("这个变量的作用范围既可以是整个系统，也可以是当前会话，具体参考："),e("a",f,[i("https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html"),l(n)])])]),h]),_,e("ol",E,[e("li",null,[i("in与exist（各有适用场景："),e("a",L,[i("in和exist的选择"),l(n)])]),y]),I,e("p",null,[e("a",w,[i("https://www.jb51.net/article/222444.htm"),l(n)])]),R,e("p",null,[e("a",N,[i("https://blog.csdn.net/horses/article/details/106905110"),l(n)])]),A,e("p",null,[i("方案一： select * from emp A where A.depNo in (select B.deptNo from dept B where "),e("a",x,[i("B.name"),l(n)]),i(" like '%技术%');")]),e("p",null,[i("方案二： select * from emp A where exists ( select 1 from dept B where B.deptNo = A.deptNo and "),e("a",q,[i("B.name"),l(n)]),i(" like '%技术%' )")]),T,S,U,e("blockquote",null,[e("p",null,[e("a",M,[i("https://mp.weixin.qq.com/s/KpTG1WUlcP6cyk8Y9SMcyQ"),l(n)]),e("a",k,[i("https://blog.csdn.net/qq_38011415/article/details/103304189"),l(n)])])]),B,e("ul",null,[D,P,O,C,e("li",null,[e("a",j,[i("有索引，但字符集不同"),l(n)])])]),Y,e("p",null,[e("a",X,[i("in和exist的选择"),l(n)])]),F,i(" ## 系统推荐 - [来个JVM冷知识](../../../other/来个JVM冷知识.md) - [JDK命令行工具](../../jvm/JDK命令行工具.md) - [测试两台服务器间的网速](../../unclassified/测试两台服务器间的网速.md) - [Centos离线安装Docker](../../docker/Centos离线安装Docker.md) - [Java语言完成notion笔记备份](../../../other/Java语言完成notion笔记备份.md) - [CountDownLatch源码解读](../../java-basic/CountDownLatch源码解读.md) - [JetBrains-License-Server](../../../other/JetBrains-License-Server.md) - [杂记](../../unclassified/杂记.md) "),H,W,Q,V])}const $=d(c,[["render",G],["__file","SQL优化.html.vue"]]);export{$ as default};
